...
class Fast_dLLM_QwenAttention(nn.Module):
    ...
    def baseline_forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        """Baseline forward pass without sparse cache - uses only dense KV cache."""
        ...
        # Handle block cache (diffusion within the block)
        ...
        # Dense cache only - no sparse cache logic
    ...
    def sparse_forward(
        ...
        block_past_key_values: Optional[Cache] = None,
        replace_position: Optional[int] = None,
        cache_state: Optional[int] = 0,
        delay_step: Optional[int] = 1,
        sparse_past_key_values: Optional[Cache] = None,
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        #### q_block shape: [32, 28, 32, 128] ####
        # cached_k shape: [32, 4, 32, 128]
        # cached_v shape: [32, 4, 32, 128]
        # they are from same block butdifferent head!!

        #### Sparse-dllm: batch size 1 ####
        # shape of q_block: torch.Size([1, 32, 32, 128])
        # shape of cached_k: torch.Size([1, 32, 1731, 128])
        # shape of cached_v: torch.Size([1, 32, 1731, 128])
        ...
        sparsed_k = None
        sparsed_v = None
        if cache_state == delay_step and past_key_value is not None and len(past_key_value) > self.layer_idx:
            #### Filter dense cache and store in sparse_past_key_values ####
            dense_k = past_key_value[self.layer_idx][0] # [32, 4, 32 * n, 128] dense cache
            dense_v = past_key_value[self.layer_idx][1] # [32, 4, 32 * n, 128] dense cache
        
            #### match head differences between query and cached key/value ####
            if query_states.size(1) != dense_k.size(1):
                dense_k_attn = dense_k.repeat_interleave(
                    query_states.size(1) // dense_k.size(1), dim=1
                )
            else:
                dense_k_attn = dense_k
            
            avg_q = query_states.mean(dim=-2)
            scores = torch.matmul(avg_q.unsqueeze(-2), dense_k_attn.transpose(-2, -1)).squeeze(-2)
            ...
            keep_num = int(importance.size(-1) * keep_ratio)
            _, keep_indices = torch.topk(importance, k=keep_num, dim=-1)
            # keep_indices: [batch_size, keep_num]
            
            # Use torch.gather for batched indexing
            batch_size, n_kv_heads, seq_len, head_dim = dense_k.shape
            # Expand indices: [batch_size, keep_num] -> [batch_size, n_kv_heads, keep_num, head_dim]
            indices_expanded = keep_indices[:, None, :, None].expand(batch_size, n_kv_heads, keep_num, head_dim)
            
            sparsed_k = torch.gather(dense_k, dim=2, index=indices_expanded)
            sparsed_v = torch.gather(dense_v, dim=2, index=indices_expanded)

            cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
            sparse_past_key_values.update(sparsed_k, sparsed_v, self.layer_idx, cache_kwargs)

            # update cache
            # past_key_value[self.layer_idx][0] = cached_k
            # past_key_value[self.layer_idx][1] = cached_v
        ...
        #### caching across the blocks ####
        # Initialize cache lengths (will be updated if cache exists)
        dense_len = 0
        sparse_len = 0
        
        if past_key_value is not None:
            # sin and cos are specific to RoPE models; cache_position needed for the static cache
            dense_len = past_key_value[self.layer_idx][0].shape[-2] if len(past_key_value) > self.layer_idx else 0
            sparse_len = sparse_past_key_values[self.layer_idx][0].shape[-2] if (sparse_past_key_values is not None and len(sparse_past_key_values) > self.layer_idx) else 0
            ...
            if update_past_key_values:
                ...
                if cache_state == 0: # called when decoding prompt block
                    key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
                elif cache_state == 2: # called when last decoding step.
                    past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
                    # Use sparse cache if available, otherwise fall back to dense cache
                    if sparse_past_key_values is not None and len(sparse_past_key_values) > self.layer_idx:
                        key_states = torch.cat((sparse_past_key_values[self.layer_idx][0], key_states), dim=-2)
                        value_states = torch.cat((sparse_past_key_values[self.layer_idx][1], value_states), dim=-2)
                        # When using sparse cache, the KV length differs from mask - disable mask
                        attention_mask = None
                    else: # when masking is done too early before step 1, there is no sparse cache we can use.
                        key_states = torch.cat((past_key_value[self.layer_idx][0], key_states), dim=-2)
                        value_states = torch.cat((past_key_value[self.layer_idx][1], value_states), dim=-2)
                ...
                if cache_state <= delay_step: # use dense cache
                    key_states = torch.cat((past_key_value[self.layer_idx][0], key_states), dim=-2)
                    value_states = torch.cat((past_key_value[self.layer_idx][1], value_states), dim=-2)

                else: # use sparse cache if available, otherwise dense
                    if sparse_past_key_values is not None and len(sparse_past_key_values) > self.layer_idx:
                        key_states = torch.cat((sparse_past_key_values[self.layer_idx][0], key_states), dim=-2)
                        value_states = torch.cat((sparse_past_key_values[self.layer_idx][1], value_states), dim=-2)
                        # When using sparse cache, the KV length differs from mask - disable mask
    ...
    def forward(
        ...
        delay_step: Optional[int] = 1,
        sparse_past_key_values: Optional[Cache] = None,
        use_sparse: Optional[bool] = True,
        keep_ratio: Optional[float] = 0.5,
        pool_kernel_size: Optional[int] = 3,
        **kwargs: Unpack[FlashAttentionKwargs],
    ) -> torch.Tensor:
        """Dispatcher forward - routes to baseline_forward or sparse_forward based on use_sparse."""
        if use_sparse:
            return self.sparse_forward(
                ...
                sparse_past_key_values=sparse_past_key_values,
